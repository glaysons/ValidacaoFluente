	...
	public static class ValidadorValorDiferenteExtensions
	{
		...
		public static IValidadorValor<T, K> Diferente<T, K>(this IValidadorValor<T, K> sender, Expression<Func<T, K>> campo)
		{
			...
			validador.AdicionarValidacao(() => (
				...
				() => outroCampo.Titulo, 
				c => c.CampoDeveSerDiferenteDoCampo);

			return sender;
		}

		public static IValidadorValor<T, K> Diferente<T, K>(this IValidadorValor<T, K> sender, K valor)
		{
			if (!(sender is ValidadorCampo<T, K> validador))
				throw new Exceptions.ValidadorInvalidoException();
			...
			validador.AdicionarValidacao(() => (
				...
				() => valor, 
				c => c.CampoDeveSerDiferenteDoValor);
